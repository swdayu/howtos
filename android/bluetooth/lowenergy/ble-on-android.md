...
## 3. BLE设备搜索流程
### 3.1 层次划分
上文介绍了BLE在Android上的架构及各组成模块之间的关系。接下来以BLE设备搜索为例，具体讨论BLE各个模块之间如何进行调用和回调，并详细介绍它们之间相互调用的流程。根据上文内容，一般可以将BLE涉及的各个层次划分为三大部分：第一部分是Java层，包括最上面的Java应用和其下的Java服务，它实现BLE应用和服务；第二部分是GATT接口层，包括GattJNI、GattHAL、以及BTIF，它实现Java世界与C语言世界之间的蓝牙抽象调用接口；第三部分是BLE协议栈，包含BTA和BTE，它是BLE核心规范Host部分的具体实现。
